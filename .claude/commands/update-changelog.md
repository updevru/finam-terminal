---
description: Update CHANGELOG.md, README.md, and CLAUDE.md for a new release
allowed-tools: Bash(git diff*), Bash(git log*), Read, Write, Edit, Glob
---

# Update Changelog

You are tasked with updating the project documentation for a new release.
Arguments (version and optional date): $ARGUMENTS

Follow these steps precisely:

## 1. Information Gathering

- If version is not provided in `$ARGUMENTS`, ask the user for it.
- Run `git diff main..HEAD --name-only` to identify files changed in this branch relative to main.
- Look for directories in `conductor/archive/` and `conductor/tracks/` that appear in the diff.

## 2. Data Extraction

- For each new track directory found in `archive/` and `tracks/`:
...
- Show the summary of changes made to all three files (CHANGELOG.md, README.md, CLAUDE.md) to the user.
