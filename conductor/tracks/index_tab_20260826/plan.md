...
- [ ] Task: (Red→Green) UI: вкладка подписывается на **весь** состав (окно `indexWindowSymbols` больше не нужно), обход опрашивает только то, что не покрыто живыми шардами; `shouldSkipQuotePolling` опирается на фактически живые символы
