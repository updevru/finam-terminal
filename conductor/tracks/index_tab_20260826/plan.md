...
## Phase 8: Шардирование подписки (по итогам вопроса пользователя)
Разведка на реальном API (2026-08-26, разовый скрипт, удалён) ответила на два открытых вопроса:
- **Точный лимит — 15 символов на подписку**: 15 принимается, 16 → `InvalidArgument: Maximum number of symbols exceeded`. Прежнее значение 10 было артефактом деления 20 пополам.
- **Лимит действует на подписку, а не на соединение**: 3 параллельных потока по 15 символов (45 бумаг) отдают котировки одновременно; 5 потоков по 9 — тоже.

Следствие: весь индекс покрывается 4 потоками в реальном времени, размеренный обход становится ненужным в штатном режиме и остаётся только фоллбеком для непокрытых/упавших шардов.

- [x] Task: (Red→Green) Менеджер подписки в `api/quote_stream.go` — набор символов режется на шарды по `defaultQuoteShardSize` = 15 (адаптивный кап остаётся страховкой), на каждый шард свой воркер со своим реконнектом и бэкоффом; `SubscribedSymbols()` возвращает символы **живых** шардов; общее число шардов ограничено сверху, хвост отбрасывается в порядке приоритета
  - Acceptance: интеграционные тесты — 46 символов покрываются несколькими подписками; падение одного шарда не гасит остальные; смена набора не перезапускает неизменившиеся шарды
- [x] Task: (Red→Green) UI: вкладка подписывается на **весь** состав (окно `indexWindowSymbols` больше не нужно), обход опрашивает только то, что не покрыто живыми шардами; `shouldSkipQuotePolling` опирается на фактически живые символы
  - Acceptance: при живом стриме вкладка не делает ни одного unary-вызова; котировки есть по всем 46 строкам
- [ ] Task: Документация и повторный ручной смоук
  - Acceptance: пользователь подтверждает поведение
