...
- [x] Task: (Изменение по итогам смоука) Убрать индикатор обновления из шапки TUI: шапка всегда рендерит только `Finam Terminal <version>`, без `⚡` и без подсказки «нажмите U»; уведомление о новой версии остаётся только в диалоге при запуске. Фоновая проверка и клавиша `U` сохраняются (проверка наполняет кэш для следующего запуска). Обновить документацию.
  - Acceptance: текст шапки байт-в-байт совпадает с дотрековым при любом состоянии обновления; тесты и линтер зелёные (cc8f22a)

> Запрос пользователя (2026-08-25, по итогам ручной проверки): «в шапке программы рядом с версией теперь надпись — нажмите U чтобы обновить. её нужно убрать. достаточно окошка при запуске программы». Пункт 8 спецификации (индикатор в шапке) отменён.
