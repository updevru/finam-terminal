...
## Phase 7: Документация и верификация [checkpoint: ddadd3f]
...
- [x] Task: (Изменение по итогам смоука) Убрать из индикатора в шапке подсказку «— нажмите U»: остаются текущая версия, `⚡` и номер доступной версии. Обновить документацию.
  - Acceptance: шапка с обновлением рендерится как ` Finam Terminal v0.13.0 [yellow]⚡ v0.14.0[-] `, без призыва к действию; тесты и линтер зелёные (cc8f22a, eb6cfe0)

> Запрос пользователя (2026-08-25, по итогам ручной проверки), в две итерации: сначала «в шапке программы рядом с версией теперь надпись — нажмите U чтобы обновить. её нужно убрать. достаточно окошка при запуске программы» → был убран весь индикатор (cc8f22a). Затем уточнение: «в шапке должна отображаться текущая версия, молния и последняя доступная версия. просто текст на что нажать отображать не нужно» → индикатор возвращён без призыва к действию (eb6cfe0). Пункт 8 спецификации в силе, изменена только формулировка индикатора.

- [x] Task: (Ручной смоук) Собрать `-ldflags` со заниженной версией (например, `v0.10.0`): при запуске появляется диалог с обеими версиями; «Продолжить» → TUI с `⚡`; клавиша `U` → модалка; «Обновить и перезапустить» → скачивание, замена, перезапуск, в шапке актуальная версия; отдельно проверить запуск без сети (запуск не тормозит, `[WARN]` в логе) и dev-сборку (тишина)
  - Acceptance: пользователь подтверждает поведение на своей платформе — подтверждено пользователем 2026-08-25 на windows/amd64 (e3a1d9b)
