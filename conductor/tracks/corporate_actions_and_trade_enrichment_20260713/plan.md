...
## Phase 3: Календари по акциям в профиле (дивиденды + сплиты) [checkpoint: 3518932]
...
- [x] Task: (Green) Рендер секций «Dividends»/«Splits» в `renderInfoPanel` (компактно, 3 будущих + 3 прошлых, сортировка по дате, `…`) (b445ac2)
  - Acceptance: тест профиля зелёный; для фьючерса/опциона/облигации этих секций нет

## Phase 4: Календари по облигациям в профиле (купоны/амортизация/оферты) [checkpoint: 8005e03]
- [x] Task: (Red) Расширить `ui/profile_test.go` — для облигации ожидаются секции «Coupons»/«Amortization»/«Offers» с деталями; тест падает (8e02e88)
  - Acceptance: тест фиксирует купон (ставка `%`, дата фиксации), амортизацию (процент, новый номинал), оферту (PUT/CALL, цена, окно дат)
- [x] Task: Догрузка событий по облигации через `GetBondEvents` в `loadProfileAsync` (ветка `BondFaceValue != ""`, не фатально) (5202a52)
  - Acceptance: для облигации профиль содержит `BondEvents`; сбой убирает секцию, не роняет профиль
- [x] Task: (Green) Рендер секций облигационных событий в `renderInfoPanel` по `Kind` (Coupon/Amortization/Offer, компактно, 3+3, сортировка, `…`) (79212f3)
...
- [x] Task: Обновить руководство пользователя `docs/user_manual/` — `history.md` (колонка `НКД`), `orders.md` (маркер `↳` связи родитель→порождённая заявка), `profile.md` (секции календарей: дивиденды/сплиты для акций, купоны/амортизация/оферты для облигаций), при необходимости `index.md` (пункт в «Возможности») (9587971)
...
- [x] Task: (Ручной смоук) На реальном ключе открыть профиль акции и облигации (увидеть календари), проверить НКД в History по облигационной сделке и `↳` в Orders
