...
- [ ] Task: (Red) Расширить `ui/profile_test.go` — для акции ожидаются секции «Dividends» и «Splits» (capped 3+3, подсказка `…`); тест падает
  - Acceptance: тест фиксирует наличие секций, формат строк (дата, размер/коэффициент, валюта/лот) и ограничение
- [ ] Task: Тип-гейтед догрузка дивидендов+сплитов в `loadProfileAsync` для акции (параллельно, не фатально)
