...
Релизная сборка сама проверяет наличие новых версий — раз в сутки, в фоне, не замедляя ни запуск, ни работу. Когда выходит новая версия, в шапке рядом с установленной версией загорается `⚡` и номер доступной, а при следующем запуске появляется диалог «Обновить и перезапустить / Продолжить». То же окно открывается клавишей `U` в любой момент.
...
- 🏦 Вкладка «Индекс»: состав Индекса МосБиржи (`GetConstituents`) с ценой, изменением за сессию и весом, отсортированный по весу. Пока вкладка активна, весь состав идёт в реальном времени: подписка разбивается на параллельные стримы, поэтому отдельные запросы котировок не нужны. Из любой строки — `Enter` в профиль, `A` в модалку заявки.
- 📈 Котировки в реальном времени по потоку `SubscribeQuote`: цены активного счёта и открытого профиля обновляются по мере поступления сделок, а при обрыве стрима терминал автоматически возвращается к опросу раз в 5 секунд. Сессия продлевается заранее, до истечения токена, поэтому терминал переживает и обрыв связи, и уход машины в сон.
...
- ⚡ Автообновление: фоновая проверка релизов GitHub раз в сутки, номер доступной версии в шапке и установка новой версии с перезапуском — при запуске или по клавише `U`.
