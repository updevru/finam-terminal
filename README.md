...
Релизная сборка сама проверяет наличие новых версий — раз в сутки, в фоне, не замедляя ни запуск, ни работу. Когда выходит новая версия, в шапке загорается значок ⚡, а при следующем запуске появляется диалог «Обновить и перезапустить / Продолжить». То же окно открывается клавишей `U` в любой момент.
...
- ⚡ Автообновление: фоновая проверка релизов GitHub раз в сутки, значок в шапке и установка новой версии с перезапуском по клавише `U`.
