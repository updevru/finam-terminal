...
*   **`ui/`**: Manages the Terminal User Interface.
    ...
    *   `update_indicator.go`: `SetUpdateAvailable`/`NotifyUpdateAvailable` (called from the checker goroutine; notifications after `Stop()` are dropped), `LatestVersion`, the `U`-key modal lifecycle, and `ConfirmUpdate`/`UpdateRequested` — the flag `main.go` acts on after `app.Run()` returns, since the process cannot replace itself while tview owns the terminal. **The discovered version is stored only — nothing is drawn.** The header deliberately carries no update indicator (removed on user request 2026-08-25); the user is told about a new version by the startup dialog, and `U` remains the on-demand path.
