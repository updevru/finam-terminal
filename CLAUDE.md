...
*   **`api/`**: Contains the `Client` struct and methods for interacting with the Finam gRPC services. Encapsulates the complexity of the raw API calls.
    ...
    *   `quote_stream.go` also owns the broker's undocumented subscription symbol cap: `IsSymbolLimitError`, `reducedSymbolCap`, `applySymbolCap`, `sameSymbolSet`, `QuoteSymbolCap()` and `SubscribedSymbols()` (what the stream carries after the cap — the UI needs it to know which rows still need polling).
...
24.  **Index Tab Quote Budget**
    ...
    *   **Measured cap (2026-08-26):** the broker settled at **10 symbols**. `Broker refused a 20-symbol quote subscription; retrying with at most 10` followed by a successful 10-symbol subscribe, and those ten were exactly the rows showing prices. Positions occupy the first slots, so a 46-name index can never be fully streamed — **the "zero unary calls" goal is unreachable at this cap** and the tab uses two sources.
    *   **Live stream:** a **window** of the composition (`indexStreamWindow` = 20 symbols, starting at the table's scroll offset quantised to `indexWindowStep` = 10) joins the existing subscription **only while the Index tab is active** and leaves it on exit. The quantisation is what stops a held arrow key from resubscribing once per row; the client then truncates to whatever the broker accepts.
    *   **Paced sweep (`sweepIndexQuotes`):** `SubscribedSymbols()` reports what the stream actually carries, `uncoveredIndexSymbols` is the complement, and the sweep walks it **one `LastQuote` per request, `indexSweepDelay` = 150ms apart**, repainting every `indexSweepRedrawEvery` = 8 rows. A 46-name pass takes ~7s and costs 46 requests/min against the 200/min budget. Spacing is the point: the broker refuses a *burst*, not the volume — 46 back-to-back calls earned an immediate `ResourceExhausted`, the same 46 spread out do not. `indexSweepDelay` is a var so tests can zero it. When the stream covers everything the sweep issues no request at all.
    *   **Cost bounds:** `indexPollCooldown` = 60s between automatic sweeps, tab must be on screen, and the session rate-limit latch. `shouldPollIndexQuotes` deliberately does **not** veto on stream liveness any more — a live stream leaves gaps by construction.
    ...
    *   **Positions-stream guard:** a second line of defence behind the symbol cap. If the subscription does not come up within 60s of the composition joining it (or fails 3 times) for any reason the cap negotiation does not cover, `indexStreamDisabled` latches for the session, the index symbols are dropped and the positions stream recovers on its own. Portfolio quotes always win over the showcase tab.
