...
*   **`api/`**: Contains the `Client` struct and methods for interacting with the Finam gRPC services. Encapsulates the complexity of the raw API calls.
    ...
    *   `quote_stream.go` also owns the broker's undocumented subscription symbol cap: `IsSymbolLimitError`, `reducedSymbolCap`, `applySymbolCap`, `sameSymbolSet` and `QuoteSymbolCap()`.
...
24.  **Index Tab Quote Budget**
    ...
    *   **Live stream (normal case):** a **window** of the composition (`indexStreamWindow` = 20 symbols, starting at the table's scroll offset quantised to `indexWindowStep` = 10) joins the existing subscription **only while the Index tab is active** and leaves it on exit, so the tab costs **zero unary quote calls** and an unwatched tab costs nothing at all. The quantisation is what stops a held arrow key from resubscribing once per row.
    *   **Stream down:** one `LastQuote` batch over the same window on tab entry, after the composition loads, and on `R`; automatic batches are spaced by `indexPollCooldown` = 60s and only run with the tab open. A batch over the whole 46-symbol composition is what the broker answered with `ResourceExhausted: Too Many Requests` in the first smoke test, so the window bounds the burst as well as the subscription. Periodic polling of the whole tab was rejected by arithmetic: 46 × 12 = 552/min.
