...
| **НКД** | Накопленный купонный доход по облигационной сделке вместе с валютой (например, `12.34 RUB`); для не-облигационных сделок колонка пустая |
...
- Колонка **НКД** заполняется только для облигаций — она показывает «грязную» составляющую цены (накопленный купонный доход); у акций и других инструментов она остаётся пустой
