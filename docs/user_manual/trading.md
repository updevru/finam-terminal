...
- **Lot Size** — размер лота отображается в подписи к полю количества: `Lots (size - 10):`

> **Какой лот используется.** В подписи показывается торговый лот (`Trade Lot` в [профиле инструмента](profile.md#торговые-параметры)) — именно на него умножается введённое количество лотов при отправке заявки. Если брокер не сообщает торговый лот, используется справочный размер лота бумаги. Для «холодного» инструмента, открытого прямо из [поиска](search.md), значение может подставиться с небольшой задержкой — подпись обновится сама, как только приложение получит его от брокера. Перед подтверждением заявки сверяйтесь с подписью: заявка уйдёт на `количество × размер лота` бумаг.
...
Когда стоп-заявка срабатывает, она порождает биржевую заявку. Пока обе заявки активны, во вкладке [«Заявки»](orders.md#связь-родитель--порождённая-заявка) порождённая заявка показывается сразу под родительской с отступом и маркером `↳`.
...
| [← Профиль инструмента](profile.md) | [Далее: Обновление программы →](updates.md) |
|:---|---:|
