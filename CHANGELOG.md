...
- **Sharded quote subscription**: measured against the real API, one `SubscribeQuote` subscription accepts exactly 15 symbols and the limit applies per subscription rather than per connection. The client now splits the symbol set across parallel streams — one worker per shard, each with its own reconnect — so the whole index is realtime at zero unary calls, and a failing shard cannot take the others down. `SubscribedSymbols()` reports the live symbols, and quote polling resumes only for positions their shard is not delivering (`index_tab`).
- **Index tab quote coverage**: rows the streams do not cover are filled by a paced sweep — one `LastQuote` per request, 150ms apart, at most once a minute and only while the tab is open. It goes quiet entirely once the streams cover the composition. The broker refuses a burst rather than the volume, so spacing keeps the requests well inside the budget (`index_tab`).
...
- **`GetSnapshots` deadlines**: every `LastQuote` now carries its own deadline instead of the whole loop sharing one context, matching the fix already made to `GetQuotes` (`index_tab`).
