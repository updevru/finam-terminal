...
- **Index tab quote coverage**: the broker accepts about ten symbols per subscription, so a 46-name index cannot be streamed in full. The tab now pairs the stream with a paced sweep — one `LastQuote` per request, 150ms apart — that fills exactly the rows the subscription does not carry, filling the table in about seven seconds and refreshing once a minute. The broker refuses a burst rather than the volume, so spacing keeps the same 46 requests well inside the budget (`index_tab`).
